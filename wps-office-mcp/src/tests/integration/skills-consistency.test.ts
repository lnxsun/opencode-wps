/**
 * Skills/Agents 一致性测�?
 * 验证 Skills �?Agents 中引用的所有工具名称在 TOOLS_INDEX 中存�?
 *
 * @date 2026-05-18
 */
...
// Legacy 工具名称映射：旧�?wps_office_* 工具�?-> gateway index 中的实际工具�?
...
describe('Skills 引用的工具名称一致�?, () => {
  ...
  skillFiles.forEach(skillPath => {
    ...
    it(`文件 ${skillPath} 不应引用不存在的工具`, () => {
      ...
      // 提取所有反引号包裹的工具名�?
      // 排除：纯下划线（�?___）、布局类型（如 title_content）、动画类型（�?fly_in�?
      const toolNamePattern = /`(\w+(?:_\w+)+)`/g;
      const matches = new Set<string>();
      let m;
      while ((m = toolNamePattern.exec(content)) !== null) {
        const name = m[1];
        // 排除纯下划线、布局类型（含有常见分隔符）、动画类�?
        if (/^_+$/.test(name)) continue;  // 纯下划线�?___
