...
- **第 4 层 MCP (COM 桥接)** — 235 个 TypeScript handler（优先执行，参数校验+类型安全）+ 240 个 COM Actions（Gateway 按需加载，无 handler 时透传 PS1）
...
| **注册工具** | 235 | `wps_xxx_xxx`（Excel 82 / Word 32 / PPT 112 / Common 9） | → Gateway 路由 | 通过 `tools/index.ts` 注册，有完整的 TypeScript handler（参数校验+类型安全）。**不注册到 MCP**，而是由 Gateway 优先调用 |
...
**三层分工**：内置工具处理基础操作，注册工具提供类型安全的深度控制，COM_ACTIONS 覆盖 240 个 WPS API 作为兜底。总计 489 个可用工具。
...
| 组件 | 说明 |
|------|------|
| **opencode-wps** | WPS JS 加载项，在 WPS 功能区添加 "OpenCode AI" 标签页，提供侧边栏 Chat UI（含 Agent 选择、模型搜索） |
...
9. **Gateway 按需加载** — 将全部 254 个工具一次性注册改为 Gateway 模式（启动时仅注册 14 内置 + 2 Gateway 工具，240 COM Actions 按需发现），大幅减少 MCP 启动耗时、节省 token
