...
│   ├── wps-ppt/               # PPT 操作技能
│   └── wps-office/            # WPS 通用技能
├── wps-office-mcp/            # 第 4 层：MCP 服务器（COM 桥接 → WPS API，240 个 COM Actions）
│   └── src/                   # TypeScript 源码
...
- **第 4 层 MCP (COM 桥接)** — 通过 PowerShell 调用 WPS COM 接口，240 个 COM Actions（通过 Gateway 按需加载）
...
| **注册工具** | 231 | `wps_xxx_xxx`（Excel 82 / Word 28 / PPT 112 / Common 9） | 直接 MCP 调用 | 通过 `tools/index.ts` 注册，有完整的 TypeScript 定义和 handler |
| **COM_ACTIONS** | 240 | 短名称（`getCellValue`, `setFont`, `addSlide`） | `wps_office_search` → `wps_office_execute` | Gateway 索引，按需发现和执行。不与注册工具重叠（不同命名） |

**三个层级互不重叠**（命名不同，执行路径不同），总计可用工具数：14 + 231 + 240 = **485**。
...
| **wps-office-mcp** | MCP (Model Context Protocol) 服务器，让 AI 通过 MCP 协议直接操作 WPS 文档（14 内置 + 231 注册 + 240 COM Actions，Gateway 按需加载） |
