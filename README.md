...
- **第 4 层 MCP (COM 桥接)** — 231 个 TS handler（优先执行，参数校验+类型安全）+ 240 个 COM Actions（Gateway 按需加载，无 handler 时透传 PS1）
...
| **注册工具** | 231 | `wps_xxx_xxx`（Excel 82 / Word 28 / PPT 112 / Common 9） | → Gateway 路由 | 通过 `tools/index.ts` 注册，有完整的 TypeScript handler（参数校验+类型安全）。**不注册到 MCP**，而是由 Gateway 优先调用 |
...
**三层分工**：内置工具处理基础操作，注册工具提供类型安全的深度控制，COM_ACTIONS 覆盖 240 个 WPS API 作为兜底。总计 485 个可用工具。
