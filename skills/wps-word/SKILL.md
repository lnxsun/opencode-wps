...
| 13 | `wps_word_enable_track_changes` | 开启/关闭修订模式（**校对前必须先开启**） |
| 14 | `wps_word_get_track_changes_status` | 获取修订模式状态和当前修订数量 |
| 15 | `wps_word_replace_range` | 按字符位置范围精确替换文本（修订模式下跟踪） |
| 16 | `wps_word_proofread_basic` | **零 token 基础校对**：用正则规则检测错别字/语病 |
