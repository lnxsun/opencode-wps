...
**2c. replaceRange 修复（禁用 findReplace）：**
...
| 规则 | 触发条件 | 拦截行为 |
|------|---------|---------|
| 批次大小 | `getDocumentParagraphs` 请求 >200 段 | ❌ 拒绝 |
| 批次连续性 | 下一批不从上一批结束段 +1 开始 | ❌ 拒绝 |
| startOffset | `proofreadBasic` 缺少或传错 startOffset | ❌ 拒绝 |
| 调用顺序 | `replaceRange` 前未调 `proofreadBasic` | ❌ 拒绝 |

这意味着 SKILL.md 中的分批规章现在有代码层强制执行。
